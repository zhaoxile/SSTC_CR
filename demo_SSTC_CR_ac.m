...
%% Load initial data
load('africa_ROIs1970_21_15_ac.mat');
Y     = Clean;
for i=1:3
    Y(:,:,:,i)=Y(:,:,:,i).*Mask(:,:,i)+1-Mask(:,:,i);
end

%%
i  = 1;
Re_tensor{i} = Y;
[psnr_0_1,ssim_0_1,sam_0_1] = HSIQA(255*Y(:,:,:,1),255*Clean(:,:,:,1));
[psnr_0_2,ssim_0_2,sam_0_2] = HSIQA(255*Y(:,:,:,2),255*Clean(:,:,:,2));
[psnr_0_3,ssim_0_3,sam_0_3] = HSIQA(255*Y(:,:,:,3),255*Clean(:,:,:,3));
nrmse_0 = sqrt(norm(Y(:)-Clean(:))^2/numel(Y));
...
if EN_SSTC_CR
    ...
    t0= tic;
    [SSTC, Out]     = SSTC_CR(Y, Mask, opts);
    time(i)         = toc(t0);
    Re_tensor{i}    = SSTC;
    [psnr_SSTC_1,ssim_SSTC_1,sam_SSTC_1] = HSIQA(255*SSTC(:,:,:,1),255*Clean(:,:,:,1));
    [psnr_SSTC_2,ssim_SSTC_2,sam_SSTC_2] = HSIQA(255*SSTC(:,:,:,2),255*Clean(:,:,:,2));
    [psnr_SSTC_3,ssim_SSTC_3,sam_SSTC_3] = HSIQA(255*SSTC(:,:,:,3),255*Clean(:,:,:,3));
    nrmse_SSTC = sqrt(norm(SSTC(:)-Clean(:))^2/numel(SSTC));
