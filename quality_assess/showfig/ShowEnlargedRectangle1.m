function I_rgb = ShowEnlargedRectangle1(I, LeftUpPoint, RightBottomPoint, Enlargement_Factor, LineWidth, gap)
...
EnlargedShowStartColumn = column - gap - LineWidth;
for j = 1 : c
%     I_rgb(EnlargedShowStartRow - m + 1:EnlargedShowStartRow,EnlargedShowStartColumn:EnlargedShowStartColumn + n - 1,j) = Enlarged(:,:,j); 
    I_rgb(EnlargedShowStartRow - m + 1:EnlargedShowStartRow,EnlargedShowStartColumn - n + 1:EnlargedShowStartColumn,j) = Enlarged(:,:,j); 
end

%% 对放大显示后的区域画矩形
Point1 = [EnlargedShowStartRow - m + 1 - LineWidth, EnlargedShowStartColumn - n + 1 - LineWidth];
Point2 = [EnlargedShowStartColumn - LineWidth, EnlargedShowStartRow - LineWidth];
