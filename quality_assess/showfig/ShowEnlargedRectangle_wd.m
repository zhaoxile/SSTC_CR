function I_rgb = ShowEnlargedRectangle_wd(I, LeftUpPoint, RightBottomPoint, Enlargement_Factor, LineWidth, gap)
...
gap2 = 8;
for j = 1 : c
    I_rgb(row + 1: row+m+gap + 2*LineWidth+gap2 , : , j)=ones(m+gap + 2*LineWidth+gap2,colum);
    I_rgb(EnlargedShowStartRow + 1+gap2:EnlargedShowStartRow + m+gap2,EnlargedShowStartColumn:EnlargedShowStartColumn + n - 1,j) = Enlarged(:,:,j); 
end

%% 对放大显示后的区域画矩形
Point1 = [EnlargedShowStartRow+1-LineWidth+gap2 , EnlargedShowStartColumn-LineWidth];
Point2 = [EnlargedShowStartRow+m+1+gap2 , EnlargedShowStartColumn+n];
% Point1 = [row+m+gap + LineWidth, row + 1];
% Point2 = [EnlargedShowStartColumn + 1,EnlargedShowStartColumn + n -1 + 1];
